...
def run_backtest(bars, entry_idx, entry_price, hold_days=20, stop_loss=-10.0, trailing_stop=-8.0, board_type="main", peak_exit=False, is_v1=False, d1_limit_up=None, buy_mode="signal_close"):
    ...
    # buy_mode 决定第一个持仓日与 peak 起点:
    #  - signal_close: 在 entry_idx 当日【收盘】买入 → 当日高低均为买入前, 不能算持仓, 跳过;
    #    第一个持仓日 = entry_idx+1(D1); peak 从买入价起 (不再把买入前 high 当峰值)。
    #  - next_open:    在 entry_idx 当日【开盘】买入 → 当日 high 为买入后, 可入 peak;
    #    第一个持仓日 = entry_idx (行为与原版一致)。
    if buy_mode == "signal_close":
        first_idx = entry_idx + 1
    else:
        first_idx = entry_idx
        if first_idx < len(bars) and bars[first_idx]['high'] > peak:
            peak = bars[first_idx]['high']

    for d in range(1, hold_days + 1):
        idx = first_idx + d - 1  # signal_close: d=1→D1; next_open: d=1→买入日
        ...
        # ② 追踪止损
        if (buy_mode == "signal_close" or d > 1) and b['low'] <= peak * (1 + trailing_stop / 100):
...
def strategy_dragon_callback(bars, code, min_pullback_days=3, max_pullback_days=11,
    ...
    for lu_idx in limit_ups:
        ...
        result = run_backtest(bars, entry_idx, entry_price, hold_days, stop_loss, trailing_stop, board_type, peak_exit=True, d1_limit_up=d1_limit_up_val, buy_mode=buy_mode)
...
def strategy_4in1(bars, code,
    ...
    for lu_idx in limit_ups:
        ...
        # ==================== 阶段1: V1 ====================
        # [去前视] 原条件 `d1_limit_up and d1_change >= vp['d1_min_change']` 偷看 D1 收盘(决策时点不可知),
        #          已改为仅由 D0 子条件(上影线/量比/跳空/EMA/RSI/换手/板块)决定, 与独立 V1 一致。
        if True:  # 直接进入 V1 子条件判定; D1 数据仅用于报告字段与出场模拟(在其自然时间点消费)
            # D0 上影线
        ...
        # ==================== 阶段2: 断板 (D1~D2) ====================
        if trade is None:  # [去前视] 移除 not d1_limit_up 门槛(D1 不可知); 断板阶段遇 D1 涨停会自然跳过(自保护)
...
def strategy_v1(bars, code, min_vol_ratio=1.5, max_upper_shadow=0.5,
    ...
    for i in range(2, len(bars)):
        ...
        # [去前视] 原在此用 D1 收盘涨幅(d1_change)过滤信号 —— D1 收盘在 D0 收盘/D1 开盘决策时不可知, 属上帝视角, 已移除。
        #          d1_change 仅保留为报告字段(result['d1_change'])。
        ...
        if buy_mode == "next_open":
            ...
            if d1_gap < min_d1_gap:
                continue
            # [去前视] 原在此用 D1 收盘涨跌(d1_change<0)排除 next_open 信号 —— D1 收盘不可知, 已移除。
        ...
        bt = run_backtest(bars, entry_idx, entry_price, hold_days, stop_loss, trailing_stop, board_type, is_v1=True, d1_limit_up=d1_limit_up_val, buy_mode=buy_mode)
        if not bt: continue
