...
def analyze_event(bars, lu_idx, code, name, circ_shares, p=QUALITY_PARAMS, verbose_feats=False):
    ...
    u['U2_turnover'] = f['turnover'] is not None and f['turnover'] >= p['u_turnover_min']
    u['U3_float_mv'] = f['float_mv'] is not None and p['u_float_mv_min'] <= f['float_mv'] <= p['u_float_mv_max']
...
def main():
    ...
    if args.from_json:
        ...
        for e in events:
            ...
            u['U2_turnover'] = e['turnover'] is not None and e['turnover'] >= QUALITY_PARAMS['u_turnover_min']
            u['U3_float_mv'] = e['float_mv'] is not None and \
