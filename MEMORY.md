...
### CSV 数据质量问题排查（15:37-16:30）
- **旧 CSV 有严重问题**：窗口导出模式 + 无停牌复牌过滤 → 创业板-60%假亏损
- **修复1：停牌复牌过滤**：`open_gap > threshold*2` 跳过（strategy_dragon_filter.py）
- **修复2：export_dragon_runs.py bug**：min-streak默认改1、多连板段元数据修复
- **修复3：optimize_mainboard.py** 同步加停牌复牌过滤

### 修复后回测结果（新CSV，full-history导出）
- 2026-01~2026-05: 43笔 62.8% +3.55% 盈亏比1.60（接近db的50-52%）
- 全量2024-01~2026-05: 249笔 42.6% -0.11% 盈亏比1.30
- **按年差异巨大**：2024:37% 2025:40% 2026:63%
- **不是数据质量问题，是市场环境差异**：弱市连板策略失效

### 新股过滤发现
- 信号日前数据<=20天：59笔 胜率仅29%（vs 整体43%）
- 需要加新股/次新股过滤

### 止损滑点问题
- 止损10%触发但实际均值亏-14.23%（A股T+1 + 跌停限制）
- 2024年止损51笔胜率仅31%，2026年止损仅7笔
- 考虑：降低止损阈值或用盘中价止损
