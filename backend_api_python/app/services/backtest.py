...
class BacktestService:
    ...
    @staticmethod
    def _strategy_has_own_exit_logic(code: str) -> bool:
        """
        Auto-detect whether strategy code already handles its own exit logic
        (stop-loss, trailing stop, take-profit, or close signals based on price conditions).

        If True, the engine should NOT overlay its own TP/SL/Trailing on top.
        If False, the engine applies TP/SL/Trailing from strategy_config.

        Detection patterns:
        1. 4-way exit signal types: close_long_stop, close_long_profit, close_long_trailing, etc.
           (generated by StrategyCompiler's core loop — always has built-in risk management)
        2. Price-based exit conditions in user-written indicator code:
           - Stop-loss: compares price to entry_price * (1 - pct) or similar
           - Trailing: tracks peak/highest/lowest then exits on drawdown
           - Take-profit: exits when return > threshold
           - Hold-time exit: exits after N bars
        """
        ...
        # Pattern 1: Compiled strategy exit signal types (StrategyCompiler output)
        # These are only generated by code that has its own core loop with TP/SL/Trailing.
        compiled_exit_patterns = [
            'close_long_stop', 'close_long_profit', 'close_long_trailing',
            'close_short_stop', 'close_short_profit', 'close_short_trailing',
        ]
        if any(p in c for p in compiled_exit_patterns):
            return True

        # Pattern 2: Stop-loss logic (price-based exit condition)
        sl_patterns = [
            'stop_loss', 'stoploss', 'stop-loss',
            'sl_price', 'sl_pct', '止损',
        ]
        has_sl = any(p in c.lower() for p in sl_patterns)

        # Pattern 3: Trailing stop / peak tracking
        trailing_patterns = [
            'trailing', 'highest', 'peak', 'drawdown',
            '追踪', '移动止盈', '回撤',
        ]
        has_trailing = any(p in c.lower() for p in trailing_patterns)

        # Pattern 4: Take-profit logic
        tp_patterns = [
            'take_profit', 'takeprofit', 'take-profit',
            'tp_price', '止盈',
        ]
        has_tp = any(p in c.lower() for p in tp_patterns)

        # Pattern 5: Hold-time / max-days exit (common in A-stock strategies)
        hold_patterns = [
            'hold_days', 'max_hold', '持仓天数', 'exit_day',
        ]
        has_hold = any(p in c.lower() for p in hold_patterns)

        # Pattern 6: Explicit close signal with price condition
        # e.g. "df.loc[condition, 'sell'] = True" where condition involves price
        # or "close_long_signals[i] = True" in a loop with price checks
        price_exit_patterns = [
            "df['close_long']", "df['close_short']",
            'close_long_signals', 'close_short_signals',
        ]
        has_price_exit = any(p in c for p in price_exit_patterns)

        # Require at least 2 independent signals to confirm strategy handles its own exits.
        # This avoids false positives from comments or parameter names alone.
        signal_count = sum([has_sl, has_trailing, has_tp, has_hold, has_price_exit])

        # Single strong signal: price-based close signal arrays (very specific, unlikely false positive)
        if has_price_exit:
            return True

        # Two or more risk-management signals present
        if signal_count >= 2:
            return True
    ...
    def run_multi_timeframe(
        ...
        # Auto-detect if strategy handles its own exits (TP/SL/Trailing).
        _handles_exits = self._strategy_has_own_exit_logic(indicator_code)
        if _handles_exits:
            logger.info("Strategy code detected to have its own exit logic — engine TP/SL/Trailing disabled (MTF)")
        if strategy_config is None:
            strategy_config = {}
        strategy_config['_strategy_handles_exits'] = _handles_exits
    ...
    def _simulate_trading_mtf(
        ...
        cfg = strategy_config or {}
        _skip_engine_risk = bool(cfg.get('_strategy_handles_exits'))
        ...
        # Risk mode: 'engine_managed' (default) or 'strategy_only'
        ...
        for i, (timestamp, row) in enumerate(df_exec.iterrows()):
            ...
            for path_price in price_path:
                ...
                # Skipped when _skip_engine_risk (strategy file handles its own exits)
                if not _skip_engine_risk and position != 0 and position_type in ['long', 'short']:
                    triggered = False
    ...
    def _run_script_strategy(
        ...
        *,
        ...
        # Script strategies always handle their own exit logic (on_bar with ctx.buy/sell/close_position)
        if strategy_config is None:
            strategy_config = {}
        strategy_config['_strategy_handles_exits'] = True
    ...
    def run(
        ...
        # Auto-detect if strategy handles its own exits (TP/SL/Trailing).
        # If so, engine skips its own risk management overlay.
        _handles_exits = self._strategy_has_own_exit_logic(indicator_code)
        if _handles_exits:
            logger.info("Strategy code detected to have its own exit logic — engine TP/SL/Trailing disabled")
        if strategy_config is None:
            strategy_config = {}
        strategy_config['_strategy_handles_exits'] = _handles_exits
    ...
    def _simulate_trading_new_format(
        ...
        cfg = strategy_config or {}
        # Auto-detect: skip engine TP/SL/Trailing if strategy code handles its own exits
        _skip_engine_risk = bool(cfg.get('_strategy_handles_exits'))
        exec_cfg = cfg.get('execution') or {}
        signal_timing = str(exec_cfg.get('signalTiming') or 'next_bar_open').strip().lower()
        risk_cfg = cfg.get('risk') or {}
        ...
        # BUT: if strategy handles its own exits (auto-detected), skip the shift entirely.
        # The strategy's core loop already manages entry/exit timing internally,
        # and shifting would double-offset the signals + override strategy-set exit prices.
        if not _skip_engine_risk and signal_timing in ['next_bar_open', 'next_open', 'nextopen', 'next']:
        ...
        for i, (timestamp, row) in enumerate(df.iterrows()):
            ...
            # Skipped when strategy handles its own exits (auto-detected from code)
            if not _skip_engine_risk and position != 0 and position_type in ['long', 'short']:
            ...
            if position > 0 and (close_long_arr[i] or _ashare_pending_exit) and not _exit_blocked:
                ...
                # When strategy handles its own exits, always prefer strategy-set price
                if _skip_engine_risk:
                    target_price = close_long_price_arr[i] if close_long_price_arr[i] > 0 else close
                elif signal_timing in ['next_bar_open', 'next_open', 'nextopen', 'next']:
                    target_price = open_
                else:
                    target_price = close_long_price_arr[i] if close_long_price_arr[i] > 0 else close
            ...
            if open_long_arr[i] and not _entry_blocked and (position == 0 or (both_mode_active and position < 0)) and capital >= min_capital_to_trade:
                    ...
                    # When strategy handles its own exits, prefer strategy-set entry price
                    if _skip_engine_risk:
                    ...
                    # Skipped when _skip_engine_risk (strategy file handles its own exits)
                    if not _skip_engine_risk and position_type == 'long' and position > 0:
            ...
            # A 股: 跌停封板时做空开仓也受影响
                    ...
                    # Strict intrabar stop-loss / liquidation check right after entry (closer to live trading).
                    # Skipped when _skip_engine_risk (strategy file handles its own exits)
                    if not _skip_engine_risk and position_type == 'short' and position < 0:
                        sl_price = entry_price * (1 + stop_loss_pct_eff) if stop_loss_pct_eff > 0 else None
            ...
            # Check if liquidation hit (safety net — always active to prevent negative equity)
            ...
            if position != 0 and not is_liquidated:
                if position_type == 'long' and liquidation_price > 0 and low <= liquidation_price:
                    if _skip_engine_risk:
                        # In strategy_only mode: force liquidation at liquidation price (no SL comparison)
                        is_liquidated = True
                        liquidation_loss = self._liquidation_loss(capital)
                        capital = 0
                        trades.append({
