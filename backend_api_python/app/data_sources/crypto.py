    ...
    def _ensure_markets_loaded(self) -> bool:
        ...
        try:
            ...
            if self._markets_cache:
                ...
                return True
            # markets 为空，加载失败，不标记为已加载，允许下次重试
            logger.debug(f"Markets loaded but empty for {self.exchange.id}")
            return False
        ...
        try:
            ...
            if klines:
                ...
                try:
                    ...
                    first_ts = _dt.fromtimestamp(klines[0]['time'], tz=timezone.utc).isoformat()
                    last_ts = _dt.fromtimestamp(klines[-1]['time'], tz=timezone.utc).isoformat()
                    logger.info(
    ...
    def _fetch_ohlcv(
        ...
        try:
            if before_time:
                ...
                while current_since < end_ms and page < max_pages:
                    page += 1
                    ...
                    # 如果最后一条数据时间超过了结束时间，说明已经获取完毕
                    if last_timestamp >= end_ms:
                        break

                    # 防止死循环：如果 timestamp 没有前进，跳出
                    next_since = last_timestamp + TIMEFRAME_SECONDS.get(timeframe, 86400) * 1000
                    if next_since <= current_since:
                        logger.warning(f"Pagination stalled at timestamp {last_timestamp}, stopping")
                        break
            ...
            # 符号不存在类错误不需要 fallback，直接返回空
            is_symbol_error = any(kw in error_str for kw in [
                'does not have market symbol', 'symbol not found',
                'invalid symbol', 'market does not exist', 'trading pair not found'
            ])
            if is_symbol_error:
                logger.warning(f"CCXT symbol not found: {symbol_pair} on {self.exchange.id}")
                return []
            # 网络/超时类错误尝试 fallback（仅用 since 参数重试一次）
