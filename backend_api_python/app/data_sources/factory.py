...
class InflightDedup:
    ...
    实现原理:
      - 使用 inflight 字典记录正在处理的请求 (key → Future)
      - 新请求先查 inflight: 已有则等 Future 结果，没有则提交新任务
      - 完成后从 inflight 清理，后续请求可以提交新任务

    Args:
        max_workers: 内部线程池最大并发数
    """
