...
class CNStockDataSource(BaseDataSource):
    ...
    def get_cnstock_selection(self, **kwargs) -> List[Dict[str, Any]]:
        """从 PostgreSQL cnstock_selection 表获取选股数据

        Kwargs:
            trade_date: 指定交易日期 (YYYY-MM-DD)，默认最新
            conditions: dict 筛选条件 {column: value}
            order_by: 排序字段
            limit: 限制条数
        ...
        order_by = kwargs.get("order_by", "change_rate")
        limit = kwargs.get("limit", 100)

        # 白名单校验
        allowed_order = {
            "code", "name", "new_price", "change_rate", "volume_ratio",
            "turnoverrate", "industry", "pe9", "pbnewmrq", "total_market_cap",
            "free_cap", "roe_weight", "sale_gpr", "netprofit_yoy_ratio",
            "debt_asset_ratio", "net_inflow", "volume", "deal_amount",
        }
        if order_by not in allowed_order:
            order_by = "change_rate"
        ...
        try:
            ...
            logger.error(f"get_cnstock_selection failed: {e}")
            return []
