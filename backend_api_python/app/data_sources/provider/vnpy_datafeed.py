# -*- coding: utf-8 -*-
"""
vnpy.datafeed 数据源 Provider

模块职责:
  通过 vnpy 数据服务接口获取 A股的 K线和实时行情数据。
  vnpy 是国内知名量化交易框架，其数据服务接口成熟稳定，作为A股数据源（priority=50）。
...
在架构中的位置:
  KlineService → DataSourceFactory → Coordinator → VnpyDataSource（本模块）
...
_vnpy_limiter = RateLimiter(
    min_interval=1.0,
    jitter_min=0.5,
    jitter_max=1.5,
)

_vnpy_quote_limiter = RateLimiter(
    min_interval=0.6,
    jitter_min=0.2,
    jitter_max=1.0,
)
...
# vnpy 代码格式: 600519.SH / 000001.SZ
def _to_vnpy_code(code: str) -> str:
    """
    将股票代码转换为 vnpy 格式: 600519.SH / 000001.SZ。

    Args:
        code: 任意格式的股票代码

    Returns:
        vnpy 格式代码，无法识别返回空字符串
    """
...
class VnpyDataSource:
    """
    vnpy.datafeed 数据源 — 量化框架数据服务（priority=50）。
    ...
    def fetch_kline_batch(
        ...
        """批量K线 — vnpy 不支持原生批量，返回 NotSupportedResult"""
    ...
    def fetch_kline(
        ...
        通过 vnpy 数据服务获取K线数据。
        vnpy 的 K线数据通常通过其 DataFeed 接口获取，
        底层可对接 RQData/Tushare/Wind 等数据源。
        ...
        try:
            ...
            logger.warning("[vnpy K线] 请求失败 %s: %s", code, e)
            return []
    ...
    def fetch_quote(self, code: str, timeout: int = 8) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.warning("[vnpy 行情] 请求失败 %s: %s", code, e)
            return None
        ...
        return {
            "symbol": vnpy_code, "name": str(d.get("f58", "")).strip(),
    ...
    def fetch_quotes_batch(self, codes: List[str], timeout: int = 10) -> Dict[str, Dict[str, Any]]:
        ...
        try:
            ...
            logger.warning("[vnpy 批量行情] 请求失败: %s", e)
            return {}
