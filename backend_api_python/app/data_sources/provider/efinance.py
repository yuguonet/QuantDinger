...
_efinance_limiter = RateLimiter(min_interval=1.0, jitter_min=0.5, jitter_max=1.5)
_efinance_quote_limiter = RateLimiter(min_interval=0.6, jitter_min=0.2, jitter_max=1.0)

_EF_KLT = {"1m": 1, "5m": 5, "15m": 15, "30m": 30, "1H": 60, "1D": 101}
...
class EfinanceDataSource:
    ...
    capabilities = {
        "kline": True, "kline_priority": 30,
        "kline_tf": {"1m", "5m", "15m", "30m", "1H", "1D"},
        "kline_batch": True,
        "quote": True, "quote_priority": 25,
        "batch_quote": True, "batch_quote_priority": 20,
        "hk": False, "markets": {"CNStock"},
    }
        ...
        try:
            resp = requests.get("https://push2his.eastmoney.com/api/qt/stock/kline/get",
                ...
                params={"secid": secid, "ut": "fa5fd1943c7b386f172d6893dbbd1835",
                        ...
                        "klt": klt, "fqt": fqt, "end": "20500101", "lmt": min(int(count), 5000)},
                timeout=timeout)
            data = resp.json()
    ...
    def fetch_batch_quotes(self, codes, timeout=10):
        ...
        try:
            ...
            logger.warning("[efinance 批量行情] 请求失败: %s", e); return {}
