# -*- coding: utf-8 -*-
"""
efinance 数据源 Provider

模块职责:
  通过 efinance 公开 API 接口获取 A股的 K线和实时行情数据。
  efinance 底层调用东方财富接口，封装为更简洁的 REST API，作为A股数据源（priority=35）。
...
在架构中的位置:
  KlineService → DataSourceFactory → Coordinator → EfinanceDataSource（本模块）
...
_efinance_limiter = RateLimiter(
    min_interval=1.0,
    jitter_min=0.5,
    jitter_max=1.5,
)

_efinance_quote_limiter = RateLimiter(
    min_interval=0.6,
    jitter_min=0.2,
    jitter_max=1.0,
)
...
# efinance 周期映射 (底层走东财 klt 参数)
_EF_KLT = {
    "1m": 1, "5m": 5, "15m": 15, "30m": 30, "1H": 60, "1D": 101,
}

# efinance 复权映射 (底层走东财 fqt 参数)
_EF_FQT = {"": 0, "qfq": 1, "hfq": 2}


@register(priority=35)
class EfinanceDataSource:
    """
    efinance 数据源 — 东财接口封装（priority=35）。

    能力:
      - K线: 日线 + 分钟线，通过东财 kline API
      - 行情: 单只实时行情
      - 批量行情: 多只股票行情
    ...
    def fetch_kline_batch(
        ...
        """批量K线 — efinance 不支持原生批量，返回 NotSupportedResult"""
    ...
    def fetch_kline(
        ...
        if klt is None:
            return []
        fqt = _EF_FQT.get(adj, 1)

        _efinance_limiter.wait()

        # efinance 底层使用东财 push2his API
        ...
        try:
            ...
            logger.warning("[efinance K线] 请求失败 %s: %s", code, e)
            return []
    ...
    def fetch_quote(self, code: str, timeout: int = 8) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.warning("[efinance 行情] 请求失败 %s: %s", code, e)
            return None
    ...
    def fetch_quotes_batch(self, codes: List[str], timeout: int = 10) -> Dict[str, Dict[str, Any]]:
        ...
        try:
            ...
            logger.warning("[efinance 批量行情] 请求失败: %s", e)
            return {}
