...
# 是否启用本地数据仓库（可通过环境变量关闭）
_WAREHOUSE_ENABLED = os.environ.get("DATA_WAREHOUSE_DISABLED", "").lower() not in ("1", "true", "yes")
...
class DataSourceFactory2:
    ...
    def _try_local_warehouse(
        ...
        try:
            ...
            if data and len(data) >= max(limit * 0.5, 10):
                # 本地数据量足够（至少有请求量的 50% 或 10 条），直接用
                logger.debug(f"[本地仓库] 命中 {symbol} {timeframe}: {len(data)} 条")
                return data
            elif data:
                # 本地有数据但不够，记录但继续用远程补充
                logger.debug(f"[本地仓库] 数据不足 {symbol} {timeframe}: {len(data)} 条，走远程补充")
                return None
            return None
        except Exception as e:
            logger.debug(f"[本地仓库] 读取异常 {symbol}: {e}")
            return None
    ...
    def get_kline(
        ...
        try:
            ...
            # 1. 优先尝试本地数据仓库
            local_data = cls._try_local_warehouse(m, symbol, timeframe, limit, before_time, after_time)
            if local_data:
                return local_data
    ...
    def get_kline_batch(
        ...
        try:
            ...
            if _WAREHOUSE_ENABLED:
                for sym in symbols:
                    local_data = cls._try_local_warehouse(m, sym, timeframe, limit)
                    if local_data:
                        result[sym] = local_data
                    else:
                        remote_symbols.append(sym)
                if result:
                    logger.info(f"[本地仓库] 命中 {len(result)}/{len(symbols)} 只 {m} {timeframe}")
            else:
                remote_symbols = list(symbols)

            # 2. 剩余的走远程 API
            if remote_symbols:
                source = cls.get_source(m)
                if hasattr(source, 'get_kline_batch'):
                    remote_data = source.get_kline_batch(remote_symbols, timeframe, limit, cached_symbols=cached_symbols)
                else:
                    remote_data = {}
                    for sym in remote_symbols:
                        try:
                            ...
                            if klines:
                                klines.sort(key=lambda x: x['time'])
                                remote_data[sym] = klines
                        except Exception as e:
                            logger.warning(f"Batch fetch failed for {m}:{sym} - {e}")
