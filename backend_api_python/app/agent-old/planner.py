...
class Planner:
    ...
    def plan_next_step(
        ...
        **kwargs,
        ...
        # 从 intent 提取 verb/noun（仅用于日志，不影响决策）
        verb = ""
        noun = ""
        if intent:
        ...
        try:
            step_data = self._llm_decide_next_step(
                query, intent, stock_code, stock_name,
    ...
    def _llm_decide_next_step(
        ...
        try:
            ...
            logger.debug("[Planner] skills 摘要加载失败，用 fallback: %s", e)
        ...
        # 对话上下文（指代消解用，如"上面那两只"、"继续分析"）
        context_section = ""
        if context_summary:
            context_section = f"\n## 上文摘要\n{context_summary}\n\n用户消息中如有指代（这些、那两只、上面、继续等），请基于上文摘要理解。\n\n"
