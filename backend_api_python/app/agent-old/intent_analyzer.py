...
@dataclass
class IntentResult:
    """意图分析结果（精简版：路由 + verb/noun + 股票 + 上下文摘要）。"""
    domain: str = "chat"
    intent: str = ""
    ...
    noun: str = ""
    context_summary: str = ""
    strategy: str = "direct"  # "traced" / "direct"

    @property
    def domain_instructions(self) -> str:
        """从 persona.md 的 behaviors 生成通用指令。"""
        try:
...
def analyze_intent(
    ...
    metadata = {"domain": domain, "intent": intent, "verb": verb, "noun": noun}

    # strategy: 金融/交易 → traced，其他 → direct
    ...
    logger.info(
        "[Intent] %s/%s (%.2f) verb=%s noun=%s strategy=%s | %s",
        domain, intent, confidence, verb, noun, strategy, message[:50],
    )
