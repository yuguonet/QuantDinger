...
# 15m 提前 prepare 的秒数（注册/刷新服务器等耗时操作提前执行）
_PREPARE_LEAD_SECONDS = 60   # 提前 1 分钟预热
...
def _next_bar_trigger() -> tuple[datetime, datetime]:
    """返回 (提前唤醒时间, 正式触发时间)。

    正式触发时间 = bar 结束前 30s
    提前唤醒时间 = 正式触发 - _PREPARE_LEAD_SECONDS（用于 prepare 注册/刷新等耗时操作）
    """
    now = datetime.now(TZ_CN)
    for h, m in _BAR_END_TIMES:
        bar_end = datetime(now.year, now.month, now.day, h, m, 0, tzinfo=TZ_CN)
        ...
        if trigger > now:
            early_wake = trigger - timedelta(seconds=_PREPARE_LEAD_SECONDS)
            return early_wake, trigger
    # 今天的 bar 都过了 → 明天第一个 bar
    tomorrow = now + timedelta(days=1)
    h, m = _BAR_END_TIMES[0]
    trigger = datetime(tomorrow.year, tomorrow.month, tomorrow.day, h, m, 0,
...
def _pre_prepare_providers():
    """提前调用所有 provider 的 prepare()（注册、刷新服务器等）。"""
    try:
...
def _scheduler_15m_loop():
    """15m 调度主循环 — 两阶段触发，永不退出。

    每个 bar 周期:
      1. 提前 _PREPARE_LEAD_SECONDS 秒醒来 → 调用 prepare() 预热
      2. sleep 到正式触发时间 → 执行 run_once 拉取数据
    """
    while True:
        try:
            # 非交易日 → sleep 到明天开盘前
            ...
            early_wake, trigger = _next_bar_trigger()

            # ── 阶段一: 提前唤醒，预热 prepare ──
            now = datetime.now(TZ_CN)
            sleep_to_early = max((early_wake - now).total_seconds(), 0)
            logger.info(
                f"[15m调度] 预热唤醒 {early_wake:%H:%M:%S}，"
                f"正式触发 {trigger:%H:%M:%S}，"
                f"sleep {sleep_to_early:.0f}s 到预热"
            )
            time.sleep(sleep_to_early)

            if not ENABLE_15M:
                continue

            logger.info("[15m调度] 预热阶段：提前调用 prepare()")
            _pre_prepare_providers()

            # ── 阶段二: sleep 到正式触发时间 ──
            sleep_to_fetch = max((trigger - datetime.now(TZ_CN)).total_seconds(), 0)
            if sleep_to_fetch > 0:
                logger.info(f"[15m调度] 预热完成，sleep {sleep_to_fetch:.0f}s 到正式触发")
                time.sleep(sleep_to_fetch)
