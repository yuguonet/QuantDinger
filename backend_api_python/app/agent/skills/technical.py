...
@skill(
    ...
    instructions=(
        ...
        "5. **用户自定义指标** — 调用 list_indicators 查看用户在指标 IDE 中创建的策略，"
        "然后用 run_indicator_signal 对目标股票执行用户指标，获取 buy/sell 信号。"
        "用户自定义指标是用户精心编写的交易策略，其信号权重应与硬编码指标同等对待。\n"
        "6. **K线形态** — 用 analyze_pattern 识别关键形态（突破、反转、整理）。\n\n"
        "A股特别注意：\n"
        "- 涨停板是极强信号，关注涨停后次日走势\n"
        "- 连板高度代表市场情绪强度\n"
        "- 换手率 > 15% 要警惕（可能见顶）\n"
