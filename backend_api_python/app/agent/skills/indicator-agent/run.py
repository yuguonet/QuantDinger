# -*- coding: utf-8 -*-
"""
indicator_agent — 指标策略执行。

评分规则：
  1. 只有今日/明日出现买卖点才评分（无信号 → 50分观望）
  2. 历史胜率/收益高 → 叠加置信度（信号质量加权）
  3. 买卖对称

数据来源：
  - indicator_analyzer.analyze_indicator() → 沙箱执行 + 信号统计 + 轻量回测
  - indicator_tools.run_indicator_signal() → 实时信号
