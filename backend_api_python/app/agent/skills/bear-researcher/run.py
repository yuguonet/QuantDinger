#!/usr/bin/env python3
"""bear-researcher: 调用工具获取数据，输出 JSON 供 agent 构建空头论据。"""
...
def main():
    ...
    for name, fn in [
        ...
        ("intel", lambda: search_comprehensive_intel(args.stock_code, query=f"{args.name or args.stock_code} 利空 风险")),
    ]:
        try:
            results[name] = fn()
    ...
    print(json.dumps({"skill": "bear_researcher", "stock_code": args.stock_code, "stock_name": args.name, "data": results}, ensure_ascii=False, indent=2))
