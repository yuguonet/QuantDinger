...
def assess_market_state() -> Dict[str, Any]:
    ...
    # 2. 涨跌停池（东财搜索，与前端一致，不依赖 scheduler 缓存）
    zt_pool = _fetch_zt_pool_em(today)
    dt_pool = _fetch_dt_pool_em(today)
    ...
    # 资金流数据不可用时，用涨停数补偿基础分
    elif net_inflow == 0 and zt_count > 0:
        mood_score += min(10, zt_count // 5)  # 每5只涨停加1分，最多+10
    if zt_count > 30:
        mood_score += 10
    elif zt_count > 15:
        mood_score += 5
...
# ═════════════════════════════════════════════════════════════════
#  东财搜索直连（与前端卡片一致，替代 scheduler 缓存）
# ═════════════════════════════════════════════════════════════════

def _em_search_direct(keyword: str, page_size: int = 100) -> List[Dict]:
    """东财智能选股搜索（与前端卡片一致）。

    前端卡片(连板天梯/强势股/热榜)均用此接口，数据一致。
    """
    from app.market_cn.eastmoney_search import search_stocks as _em_search
    from app.data_sources.normalizer import safe_float
    try:
        raw = _em_search(keyword=keyword, page_size=page_size)
        if raw.get("code") != 1:
            return []
        return [{
            "code": s.get("code", ""),
            "name": s.get("name", ""),
            "change_rate": safe_float(s.get("change_rate"), 0),
            "new_price": safe_float(s.get("new_price"), 0),
            "turnoverrate": safe_float(s.get("turnoverrate"), 0),
            "industry": s.get("industry", ""),
            "concept": s.get("concept", ""),
            "deal_amount": s.get("deal_amount"),
            "total_market_cap": s.get("total_market_cap"),
        } for s in raw.get("stocks", [])]
    except Exception as e:
        logger.warning("[MktScreen] 东财搜索 '%s' 失败: %s", keyword, e)
        return []


def _fetch_zt_pool_em(date: str) -> List[Dict]:
    """东财搜索涨停/连板数据（与前端连板天梯卡片一致，替代 scheduler 缓存）。"""
    from app.data_sources.normalizer import safe_int
    stocks = _em_search_direct("连续涨停 连板", page_size=100)
    return [{
        "stock_code": s["code"],
        "stock_name": s["name"],
        "trade_date": date,
        ...
        "price": s["new_price"],
        "continuous_zt_days": safe_int(s.get("continuous_zt_days", 1)),
        "turnover_rate": s["turnoverrate"],
    } for s in stocks]


def _fetch_dt_pool_em(date: str) -> List[Dict]:
    """东财搜索跌停数据。"""
    from app.data_sources.normalizer import safe_int
    stocks = _em_search_direct("跌停", page_size=100)
    return [{
        "stock_code": s["code"],
        "stock_name": s["name"],
        "trade_date": date,
...
def prescreen(date: str) -> Dict[str, Any]:
    ...
    logger.info("[MktScreen] 连板: %d只, 龙回头: %d只, 强势股: %d只, 热榜: %d只",
                len(continuous_board), len(dragon_pullback),
                len(strong_stocks), len(hot_stocks))
    ...
    # 排序：连板 > 龙回头 > 强势股 > 热榜 > 条件搜索
    source_priority = {"连板": 0, "龙回头": 1, "强势股": 2, "热榜": 3, "条件搜索": 4}
