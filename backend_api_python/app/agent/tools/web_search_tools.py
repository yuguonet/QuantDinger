...
def search_stock_news(stock_name: str, stock_code: str = "",
                      days: int = 7, count: int = 8) -> dict:
    """
    搜索个股相关新闻、公告、研报 — 获取某只股票的最新市场情报。
    ...
    query_parts.extend(["最新", "新闻", "公告"])
    query = " ".join(query_parts)

    freshness_map = {1: "pd", 3: "pw", 7: "pw", 15: "pm", 30: "pm"}
    freshness = ""
    for threshold, val in sorted(freshness_map.items()):
        if days <= threshold:
            freshness = val
            break
...
def search_market_news(topic: str = "A股", count: int = 8) -> dict:
    """
    搜索市场热点新闻 — 获取大盘、板块、政策等宏观市场情报。

    Args:
        topic: 搜索主题（如 "A股大盘"、"央行货币政策"、"半导体板块"）
        count: 返回结果数量，默认 8
    ...
    """
    count = min(max(count, 1), 10)
    result = _unified_search(f"{topic} 最新消息", count=count, freshness="pw")
    output = _format_output(result)
    output["topic"] = topic
    return output


def search_policy_news(policy_topic: str, count: int = 5) -> dict:
    """
    搜索政策法规新闻 — 获取最新政策、监管动态。

    Args:
        policy_topic: 政策主题（如 "降准"、"注册制"、"新能源补贴"）
        count: 返回结果数量，默认 5
    ...
    """
    count = min(max(count, 1), 10)
    result = _unified_search(f"{policy_topic} 政策 最新", count=count, freshness="pm")
    output = _format_output(result)
    output["policy"] = policy_topic
    return output
