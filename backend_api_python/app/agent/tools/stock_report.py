...
def _generate_report(
    ...
    if signal:
        ...
        signal_parts.append(f"技术面{_direction_cn(direction)}({score}分{short_signal})")
    if pe_ttm and pe_ttm < 0:
        signal_parts.append("基本面亏损")
    if fund_signal and "流出" in fund_signal:
        signal_parts.append("资金融资净流出")
    elif fund_signal and "流入" in fund_signal:
        signal_parts.append("资金净流入")
    full_signal = "+".join(signal_parts) if signal_parts else "数据不足"

    # ── 分析文本 ──
    analysis_parts = []
    if technical.get("analysis"):
        first_line = technical["analysis"].split("\n")[0] if "\n" in technical["analysis"] else technical["analysis"]
        analysis_parts.append(first_line)
    if pe_ttm and pe_ttm < 0:
        analysis_parts.append("PE为负表示亏损")
    if fund_signal:
        analysis_parts.append(fund_signal)
    analysis = "，".join(analysis_parts) if analysis_parts else "数据不足"
