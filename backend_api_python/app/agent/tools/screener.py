...
- eastmoney_screen() — 内部函数，调用东方财富 search-code API
...
TOOL_SPEC = [
    ...
    },
    {
        ...
        "description": (
            ...
            "用于了解 smart_screen 的 filters 参数的完整格式。"
        ),
        "parameters": {"type": "object", "properties": {}},
    },
    {
...
def eastmoney_screen(keyword: str, market: str = "全部", filters: Optional[Dict[str, Any]] = None,
                     page_size: int = 50, page_no: int = 1, proxy: Optional[str] = None) -> Dict[str, Any]:
    """调东财 API 选股（核心选股函数）。"""
    if filters:
...
def smart_screen(
    ...
    try:
        if mode == "eastmoney":
            return eastmoney_screen(keyword=keyword, market=market, filters=filters, page_size=top_n)
            ...
            base = eastmoney_screen(keyword=keyword, market=market, filters=filters, page_size=top_n)
            if "error" in base:
                return base
...
# 合并所有工具
TOOL_SPEC = TOOL_SPEC + BACKEND_DATA_TOOLS
