...
# ═══════════════════════════════════════════════════════════════
# 数据获取辅助（委托给 data_tools，消除重复）
# ═══════════════════════════════════════════════════════════════

def _fetch_klines(stock_code: str, days: int = 120) -> List[Dict[str, Any]]:
    """获取原始K线数据（含 OHLCV）。委托给 data_tools.get_daily_history。"""
    from app.agent.tools.data import get_daily_history
    return get_daily_history(stock_code, days=days)
...
TOOL_SPEC = [
    {
        ...
        "name": "analyze_trend",
        "description": (
            "综合技术趋势分析（MA + MACD + RSI + BOLL + KDJ 五维共振），"
            "返回均线排列、趋势评分、买卖信号和所有子指标详情。"
            "这是最核心的分析工具，一次调用获取全部技术面数据。"
        ),
        "parameters": {
            "type": "object",
            "properties": {
    ...
    },
    {
        ...
        "parameters": {
            ...
            "properties": {
                "stock_code": {"type": "string", "description": "股票代码"},
                "periods": {"type": "string", "description": "均线周期，逗号分隔，默认5,10,20,60,120", "default": "5,10,20,60,120"},
            },
            "required": ["stock_code"],
        },
    },
    ...
    },
    {
        ...
        "description": (
            "识别K线形态（增强版）：锤子线、十字星、吞没、早晨/黄昏之星、"
            "三连阳/阴、红三兵/黑三鸦、长上下影线、缺口等 15+ 种形态。"
        ),
        "parameters": {
            "type": "object",
            "properties": {
    ...
    {
        "fn": get_indicator_snapshot,
        "name": "get_indicator_snapshot",
        "description": (
            "一次性获取所有主要技术指标快照（MA/MACD/RSI/BOLL/KDJ/量比），"
            "减少多轮工具调用。适合需要快速全面了解技术面的场景。"
        ),
        "parameters": {
            "type": "object",
            "properties": {
