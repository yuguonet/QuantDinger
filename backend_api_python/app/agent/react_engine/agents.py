...
import yaml
# huggingface_hub removed — not needed
# jinja2 removed — use simple string formatting
# rich removed — replaced with plain print in monitoring.py
...
# remote_executors removed — not needed
...
from .utils import (
    ...
    # create_agent_gradio_app_template removed,
...
def populate_template(template: str, variables: dict[str, Any]) -> str:
    ...
    for key, value in variables.items():
        if key == 'tools':
            continue  # tools 由下面的循环处理
        if value is None:
            value = ''
        template = template.replace('{{' + key + '}}', str(value))
        template = template.replace('{{ ' + key + ' }}', str(value))

    # 处理 tools 循环: {%- for tool in tools.values() %}...{% endfor %}
    tools_dict = variables.get('tools', {})
    tools_desc = '\n'.join(t.to_code_prompt() for t in tools_dict.values()) if tools_dict else ''
    template = re.sub(
        r'\{%-?\s*for tool in tools\.values\(\)\s*-?%\}.*?\{%-?\s*endfor\s*-?%\}',
        lambda m: tools_desc,
    ...
    # 清理 managed agents 空段落
    template = re.sub(
        r'You can also give tasks to team members\..*?Here are the rules',
        'Here are the rules',
        template, flags=re.DOTALL
    )
    # 清理连续空行
...
{str(additional_args)}."""
    ...
    def _generate_planning_step(
        ...
        self.logger.log_rule(log_headline, level=LogLevel.INFO)
        self.logger.log(plan, level=LogLevel.INFO)
        yield PlanningStep(
...
class CodeAgent(MultiStepAgent):
    ...
    def __init__(
        ...
        code_block_tags: str | tuple[str, str] | None = "markdown",
        **kwargs,
    ):
        ...
        if self._use_structured_outputs_internally:
            prompt_templates = prompt_templates or yaml.safe_load(
                open(os.path.join(os.path.dirname(__file__), "prompts", "structured_code_agent.yaml"), encoding="utf-8").read()
            )
        else:
            prompt_templates = prompt_templates or yaml.safe_load(
                open(os.path.join(os.path.dirname(__file__), "prompts", "code_agent.yaml"), encoding="utf-8").read()
            )
    ...
    def _step_stream(
        ...
        try:
            ...
            # 如果 LLM 输出纯文本或 "Final Answer:" 格式，直接当 final_answer 返回
            stripped = output_text.strip()
            # 提取 Final Answer: 后面的内容
            answer_text = None
            for prefix in ["Final Answer:", "Final answer:", "FINAL ANSWER:"]:
                if prefix in stripped:
                    answer_text = stripped.split(prefix, 1)[1].strip()
                    # 去掉可能的代码块标记
                    for tag in ["```", "<code>", "</code>"]:
                        answer_text = answer_text.replace(tag, "").strip()
                    break
            # 或者整个输出都没有代码块
            if not answer_text and "```" not in stripped and "<code>" not in stripped:
                answer_text = stripped
            if answer_text:
                code_action = f'final_answer({answer_text!r})'
                code_action = fix_final_answer_code(code_action)
                memory_step.code_action = code_action
            else:
        ...
        try:
            ...
            if hasattr(self.python_executor, "state") and "_print_outputs" in self.python_executor.state:
                ...
                if len(execution_logs) > 0:
                    ...
                    self.logger.log("\n".join(str(x) for x in execution_outputs_console), level=LogLevel.INFO)
            error_msg = str(e)
