...
#   1 与刻意交接的数据抢 _OBJ_MAX_PER_SCOPE=200 的 FIFO 名额,长跑会挤掉跨阶段数据;
#   2 该作用域会被 auto-*.obj 噪声淹没,违背"点名即可引用";
#   3 需要 json.loads / 取回调用,多一步且易踩类型坑。
#
...
def _wrap_stage_guard(fn, tool_name, executor=None):
    ...
    **通知**而不是数据 ⇒ 模型被迫多走一步去取回(实测 Step1 发现名字、Step2 才用)。
    ...
    def wrapper(*args, **kwargs):
        ...
        for _check in (_hooks_now().get("probes") or ()):
            _check()
...
# ── 会话钩子表（2026-09-24 提智 0.7 收编，审计 B1）──────────────────────
# 原形态：_INTERRUPT_CHECKS 全局 list + _current_event_cb/_user_step_callbacks/
# _active_code_agent 实例槽 + save/restore——多 worker 并发互踩（A 收尾抹掉 B 的
# 注册 / 陈旧探针串会话误杀 / 事件串流丢失）。现按 session_id 注册 + 线程局部绑定
# 当前 run（worker 一线程一任务，绑定无歧义）；注册/注销幂等，不再互相覆盖。
_RUN_HOOKS: dict = {}          # session_id -> {event_cb, step_checks, probes, active_agent}
_RUN_HOOKS_LOCK = threading.Lock()
_RUN_TLS = threading.local()


def bind_run_session(session_id: str) -> None:
    """worker 在任务开始时绑定本线程的 run 归属（0.7）。"""
    _RUN_TLS.sid = str(session_id or "")
...
def register_run_hooks(session_id: str, *, event_cb=None, step_checks=None, probes=None) -> None:
    ...
    with _RUN_HOOKS_LOCK:
        _RUN_HOOKS[sid] = {
            ...
            "active_agent": _RUN_HOOKS.get(sid, {}).get("active_agent"),
        }


def unregister_run_hooks(session_id: str) -> None:
    with _RUN_HOOKS_LOCK:
        _RUN_HOOKS.pop(str(session_id or ""), None)


def _hooks_now() -> dict:
    """当前线程 run 的钩子快照（消费侧唯一入口）。"""
    sid = getattr(_RUN_TLS, "sid", "")
    with _RUN_HOOKS_LOCK:
        return dict(_RUN_HOOKS.get(sid) or {})


def _hooks_bind_active(agent) -> None:
    """把当前在跑的 CodeAgent 挂到本会话槽（探针按会话取，不误杀他人）。"""
    sid = getattr(_RUN_TLS, "sid", "")
    with _RUN_HOOKS_LOCK:
        _RUN_HOOKS.setdefault(sid, {})["active_agent"] = agent


_INTERRUPT_CHECKS: list = []   # [废弃 2026-09-24] 兼容读点：新代码用 _hooks_now()["probes"]
...
class _LLMAdapter:
    ...
    def generate(
        ...
        **kwargs,
        ...
        # 2026-09-24(提智阶段 0.10,审计 B9):以下两行注释与报错原文是历史编码损坏的
        # 乱码(????),按上下文语义重写(finish_reason=length = 输出被 max_tokens 截断)。
        # finish_reason=length:输出被 max_tokens 截断,模型没说完就断了--这不是内容错误
        # 而是容量不足,必须显式报错让调用方调整,不能当正常输出继续。
        if getattr(resp, "finish_reason", "") == "length":
            raise RuntimeError(
                "模型输出被 max_tokens 截断(finish_reason=length),任务未完成。"
                "请调大 CODE_AGENT_MAX_TOKENS / OPENAI_MAX_TOKENS,或简化任务缩小输出规模。"
            )
...
def _skill_run_py_exists(skill_name: str) -> bool:
    """纯 SKILL.md 技能（目录无 run.py）判定（2026-09-21）。

    约定：有的技能只提供 SKILL.md 正文（如 auto_finance-analysis-stock），
    没有 run.py 可执行模块。旧实现无条件按 `skills.<name>.run` 导入，必
    ImportError → 每次运行刷 2 条 WARNING（噪音，掩盖真实失败）。
    无 run.py 属**正常缺省路径**：静默返回，不打告警。
    """
    mod = skill_name.replace("-", "_")
    base = os.path.join(os.path.dirname(__file__), "..", "skills")
    # 目录名既可能是原始名（含连字符），也可能是下划线归一化名
    for d in (skill_name, mod):
        if os.path.isfile(os.path.join(base, d, "run.py")):
            return True
    return False
...
def _load_skill_functions(skill_name: str, skill_adapter=None) -> list:
    ...
    if not _skill_run_py_exists(skill_name):
        return []
    try:
...
def _list_skill_func_names(skill_name: str) -> set:
    ...
    纯 SKILL.md 技能（无 run.py）静默返回空集（2026-09-21）。
    """
    import importlib
    module_name = skill_name.replace("-", "_")
    if not _skill_run_py_exists(skill_name):
...
class TaskAgent(AgentBase):
    ...
    async def _plan(
        ...
        # ── 编排缓存激活(2026-09-15,闭环4):历史成功链路作为**参考**注入 plan 提示 ──
        ...
        # "空域 + 金融类意图"直接判退化并留痕，其余空域只告警（纯推理任务本就不该拉域）。
        if not selected_domain and not selected_skill:
        ...
        # ── B1 Plan Linter（R1 工具覆盖 / R2 隐式依赖，2026-09-24 提智二波）──
        # 确定性"产前检查"：① 任务书声明的数据需求是否真有对应工具落在本轮工具面内
        # （词典高置信可补位：单段补顶层点名、阶段只补"自己 goal 也命中该域"的白名单；
        # 索引候选低置信只告警）；② 下游阶段消费上游产出的清单却没标 barrier → 自动补，
        # WARN 不阻塞。设计取舍与易错点见 utils/plan_linter.py 头部。
        # 放在此处（而非 phases 归一化之前）：此时 phases/plan_tools 都是最终形态，
        # 且仍早于 `plan_tools and phases → plan_tools=[]` 的失效判定，补位才落得进去。
        try:
            ...
            if _lint.dirty:
                ...
                for _w in _lint.warnings:
                    logger.warning("[PlanLint] %s", _w)
                if _lint.phase_tool_additions:
                    logger.info("[PlanLint] 阶段白名单补位: %s", _lint.phase_tool_additions)
                if _lint.plan_tools_additions:
                    logger.info("[PlanLint] 顶层点名补位: %s", _lint.plan_tools_additions)
        except Exception as _e:
            # 咨询性检查不得阻断规划主链；但**必须发声**（不许静默降级）：warning 级留痕，
            # 若反复出现说明 linter 与契约漂移，按日志修，不要在此处加兜底分支。
            logger.warning("[PlanLint] 检查失败（已跳过，规划主链不受影响）: %s", _e)
    ...
    async def chat(
        ...
        return await self._chat_plan_graph(user_input, session_id, use_rag,
                                           event_cb=_hooks_now().get("event_cb"))
    ...
    def _build_code_agent(
        ...
        **但执行时一个都调不到**,调用即被误报成"幻觉调用"。日志现同时打印
        ...
        #   1 `GuidedCPythonExecutor._promote_model_vars()` -- 每步执行后促升到会话级;
        #   2 下方投影 -- 新建 executor 时把会话级变量装回 state。
        ...
        # list_tools 视图 ≡ 沙箱实持（2026-09-24，设计 §7.19 加固②）：在包装**之前**对
        # tool_functions 做键集+签名+描述快照（_wrap_stage_guard/breaker 包装会丢 docstring，
        # 与契约渲染同因）；包装只重绑值不改名，快照即沙箱实持面的同源视图。
        _tool_view = []
        for _n in sorted(tool_functions.keys()):
            _f = tool_functions[_n]
            try:
                _sg = _sig(_f)
            except Exception:
                _sg = ""
            _d = (getattr(_f, "description", "") or getattr(_f, "__doc__", "") or "") \
                .strip().split("\n")[0][:100]
            _tool_view.append((_n, _sg, _d))
        ...
        class _ListToolsTool(SmolToolBase):
            ...
            def forward(self, domain: str = "", **kwargs):
                # §7.19 加固②（2026-09-24）：视图 **恒等于沙箱实持**（同源派生——快照自
                # 注入用的 tool_functions 键集 `_tool_view`，见注入区）。旧实现三条件分支各自
                # 推导（whitelist 只列白名单、漏 common/技能工具；tools=None+空 domain 回退
                # "all" 列全量 58 个而沙箱只持 2 个）——正是 §7.19“提示里有、沙箱里没有”
                # 名单差的来源（09-19 事故根因之一）。domain 参数保留签名兼容，不再参与过滤。
                if not _tool_view:
                    return "本阶段无数据工具（search_tools 仍可用）。"
                lines = [f"本阶段可用工具 ({len(_tool_view)})："]
                for _n, _sg, _d in _tool_view:
                    lines.append(f"  - {_n}{_sg} — {_d}" if _d else f"  - {_n}{_sg}")
                return "\n".join(lines)
        ...
        # smolagents 里 tools= 有两个作用,本项目只吃到 1:
        # 1【生效|沙箱可调用】run 启动时随 send_tools 注入执行器
        ...
        # 2【不生效|随提示下发描述】smolagents 默认模板里有
        ...
        # 3【因此】LLM 能认识 search_tools / final_answer / 技能工具的名字,通道是
        ...
        def _truncate_tool_call_args(step: ActionStep, cap: int = 300) -> None:
            ...
            # 参数字符串随步数累积重放(实测每步 8-12k 字符,是 O(n2) 主引擎之一)。
        ...
        def _preserve_structure(memory_step: ActionStep, agent: SmolCodeAgent) -> None:
            """0.9 结构保真钩子（占位）：不启用**任何截断**——靠单块生成 + 模型自约束省 token，
            截断只作用于旧步、不动当前交付物。
            （2026-09-20 教训：截断机制与溯源门耦合 → 拒收死循环；提智评审 C9。）
            """
            return

        # B2 失败记忆（2026-09-24）：run 内闭环——采集器在沙箱登记，此处单一通道注入。
        from utils.failure_memory import FailureMemory, drain_executor_events as _drain_fm
        _failure_memory = FailureMemory()

        # B4 执行修复（2026-09-24，二波末）：**单次**、fail-open、默认关(AGENT_CODE_REPAIR)。
        # 只对可修 error_type 试一次；结果只作为【修复建议】注入观测，不越权改模型代码。
        _repair_state = {"used": False}

        def _repair_step(memory_step: ActionStep, agent: SmolCodeAgent) -> None:
            try:
                from execution.repair import (repair_enabled, repair_code,
                                              REPAIRABLE_ERROR_TYPES)
                if not repair_enabled() or _repair_state["used"]:
                    return
                ev = list(getattr(executor, "_failure_events", []) or [])
                pick = None
                for _et, _det in ev:
                    if _et in REPAIRABLE_ERROR_TYPES:
                        pick = (_et, _det)
                        break
                if pick is None:
                    return
                _repair_state["used"] = True
                try:
                    _tools = list(executor._tool_names())[:60]
                except Exception:
                    _tools = []
                _code = getattr(memory_step, "code_action", None) or ""
                _res = repair_code(_code, pick[1], error_type=pick[0], available_tools=_tools)
                if not _res.ok:
                    logger.info("[Repair] 放弃修复: %s", _res.note)
                    return
                _obs = getattr(memory_step, "observations", None)
                _hint = ("\n【修复建议】(仅供一次参考，可能仍不完整)\n"
                         "```python\n" + _res.fixed_code[:1500] + "\n```")
                if isinstance(_obs, str):
                    memory_step.observations = _obs + _hint
                logger.info("[Repair] 注入修复建议(%s) len=%d", pick[0], len(_res.fixed_code))
            except Exception as e:
        ...
        def _failure_memory_step(memory_step: ActionStep, agent: SmolCodeAgent) -> None:
            ...
            try:
                ...
                _obs = getattr(memory_step, "observations", None)
                if isinstance(_obs, str):
                    memory_step.observations = _obs + "\n" + _txt
                try:
                    agent._failure_memory = _failure_memory
                except Exception:
                    pass
                logger.info("[FailureMemory] 注入 %s", _new)
            except Exception as e:
                logger.debug("[FailureMemory] 注入跳过: %s", e)

        def _budget_step(memory_step: ActionStep, agent: SmolCodeAgent) -> None:
            """0.9 成本硬预算：每步现算 token/工具调用，超限则软收尾（不 raise）。"""
            try:
                from utils.budget import (tokens_from_agent, tool_calls_from_agent,
                                          check_budget, budget_wrapup_hint)
                _toks = tokens_from_agent(agent)
                _tools = tool_calls_from_agent(agent)
                # 日配额：按 session 记账（用户级无统一 id 入口时取会话级近似）
                _sid = getattr(agent, "_qd_session_id", "") or getattr(self, "_session_key", "")
                _day = 0
                try:
                    from utils.budget import add_day_usage
                    _day = add_day_usage("session:" + str(_sid or "default"), _toks - int(getattr(agent, "_last_counted_tokens", 0) or 0))
                    agent._last_counted_tokens = _toks
                except Exception:
                    _day = 0
                reason = check_budget(run_tokens=_toks, tool_calls=_tools, day_tokens=_day)
                _bc = getattr(agent, "_budget_counters", None)
                if _bc is None:
                    _bc = {}
                    try:
                        agent._budget_counters = _bc
                    except Exception:
                        pass
                _bc["tokens"] = _toks
                _bc["tool_calls"] = _tools
                if not reason:
                    return
                _bc["exceeded"] = reason
                if getattr(agent, "_budget_warned", False):
                    return
                try:
                ...
                if isinstance(obs, str):
                    memory_step.observations = obs + budget_wrapup_hint(reason)
                logger.warning("[Budget] 超限 %s（tokens=%d tools=%d），注入软收尾提示",
                               reason, _toks, _tools)
            except Exception as e:
        ...
        agent = SmolCodeAgent(
            ...
            step_callbacks=(
                ...
                + list(_hooks_now().get("step_checks") or ())
        ...
        # 立即停止(2026-09-17):当前在跑的 CodeAgent 挂**会话级**槽——
        # 2026-09-24 提智 0.7 收编:原全局实例槽并发互踩(A 的 CodeAgent 会被 B 的探针
        # interrupt_switch 命中而误杀);现按 session 归档,探针经 _hooks_now 取本会话的。
        # smolagents 原生 interrupt_switch 在下一个步边界抛 AgentError 中止。
        _hooks_bind_active(agent)
        self._active_code_agent = agent  # 兼容旧读点(仅排查用,勿新增消费)
    ...
    async def _chat_plan_graph(
        ...
        from nodes import (
            ...
            make_execute_node, make_finalize_node, make_verify_node,
        ...
        try:
            ...
            # E3（2026-09-24 提智先手）：execute 与 finalize 之间插入 verify 节点。
            # 只对「含数字结论」启用（L1+），其余直通——省调用、防误伤。
            ...
            # verify → finalize（PASS/DEGRADE）| execute（REPAIR_ONCE，回炉一次写死）
            ...
            result = {}
            _node_cn = {"chat": "意图解析", "plan": "任务规划", "execute": "工具执行",
                        "verify": "结果校验", "finalize": "结果整理"}
            _node_next = {"chat": route_after_chat, "plan": route_after_plan,
                          "execute": route_after_execute, "verify": route_after_verify}
