...
- 深度思考,找出最优选择
- **多步骤处理**：如果用户明确指定了步骤（如"第一步"、"第二步"、"首先"、"然后"），按用户指定的步骤拆分为多个 phase，每个 phase 对应用户指定的一个步骤

### phase 合并规则（重要）

- **不要过度拆分**：每个 phase 应该是一个有独立意义的分析阶段，而不是一个工具调用
- **相关操作合并**：同一类操作（如多个数据获取、多个技术指标计算）应合并到一个 phase 内，让 Agent 在该 phase 中自主调用多个工具
- **典型合并示例**：
  - 获取行情 + 计算指标 → 一个 phase（"技术分析"）
  - 获取资金流向 + 获取龙虎榜 → 一个 phase（"资金面分析"）
  - 搜索新闻 + 分析情报 → 一个 phase（"情报分析"）
- **拆分时机**：仅当后一步的输入依赖前一步的输出时，才拆分为独立 phase（progressive=true）
- **一般规则**：简单股票分析通常 2~3 个 phase 足够，不超过 3 个
