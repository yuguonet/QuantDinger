...
def _backtest_single_stock_all_strategies(
    ...
    """
    对单只股票串行跑所有策略，返回每个策略的结果列表。

    与 _backtest_single_stock 接口对齐，但一次调用覆盖全部策略，
    避免同一只股票被多个策略重复拉取 K 线。

    Returns:
        [{"indicator_id": int, "indicator_name": str, "result": dict}, ...]
    """
    results = []
    for sc in strategy_configs:
        ...
        bt_result = _backtest_single_stock(
            indicator_code=sc["indicator_code"],
            ...
            market=market,
            periods=sc["periods"],
            user_params=sc["params"],
            save_to_db=save_to_db,
            cancelled=cancelled,
            param_combos=sc.get("param_combos"),
            searchable_params=sc.get("searchable_params"),
        )
        results.append({
            "indicator_id": sc["indicator_id"],
            "indicator_name": sc["indicator_name"],
...
def _run_stocks_parallel_all_strategies(
    stocks: List[Dict[str, Any]],
    strategy_configs: List[Dict[str, Any]],
    save_to_db: bool,
    cancelled: List[bool],
    ...
    """
    小循环并行引擎：外层股票并行，内层策略串行。

    每只股票只拉取一次 K 线数据，然后对该股票串行跑所有策略。
    相比原来的大循环（外层策略 × 内层股票），K 线拉取次数从
    N策略 × M股票 降为 M股票。
    ...
    total = len(stocks)
    logger.info(
        f"[_run_stocks_parallel_all_strategies] workers={max_workers}, "
        f"stocks={total}, strategies={len(strategy_configs)}"
    )
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for stock in stocks:
            if cancelled[0]:
                break
            future = executor.submit(
                _backtest_single_stock_all_strategies,
        ...
        for future in as_completed(future_map):
            ...
            stock = future_map[future]
            try:
                # 每只股票跑全部策略，超时按策略数线性放大
                per_stock_timeout = max(120, len(strategy_configs) * 60)
                all_results = future.result(timeout=per_stock_timeout)
...
def backtest_all(
    ...
    try:
        # ── 小循环：外层股票并行，内层策略串行 ──
        # 每只股票只拉取一次 K 线，对该股票串行跑所有策略
        for item in _run_stocks_parallel_all_strategies(
            stocks=stocks,
            strategy_configs=strategy_configs,
            save_to_db=save_to_db,
            cancelled=cancelled,
            ...
            if item_type == "progress":
                ...
                task_idx += 1
                sid = item.get("indicator_id")
                sname = item.get("indicator_name", "")
                symbol = item.get("symbol", "")
                name = item.get("name", "")
        ...
        for sc in strategy_configs:
            ...
            s_stats = stats["by_strategy"][sid]
            yield _sse({
                "type": "strategy_done",
                "indicator_id": sid,
                "indicator_name": sname,
                "passed": s_stats["passed"],
                "skipped": s_stats["skipped"],
                "msg": f"策略 {sname} 完成：通过 {s_stats['passed']} 只",
            })
